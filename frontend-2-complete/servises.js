...
function renderServices(categoryId) {
  ...
  // Перевешиваем обработчики на новые кнопки. Старые кнопки
  // исчезли вместе со старым HTML, поэтому старые обработчики
  // тоже пропали — их надо навесить заново на новые элементы.
  setupSelectButtons();
}
...
// setupSelectButtons — вешает обработчики на кнопки «Выбрать».
//
// Вызывается после каждой перерисовки списка услуг — потому что
// кнопки создаются заново, и на них нужны новые обработчики.
//
// Логика клика:
//   1. Найти строку услуги (родителя кнопки) через closest.
//   2. Взять id из data-service.
//   3. Найти саму услугу через findServiceById.
//   4. Сохранить её в localStorage.
//   5. Перейти на страницу выбора даты.
...
// setupBackButton — кнопка «Назад» в сайдбаре ведёт на главную.
